...
//! [`MakeWriter`](tracing_subscriber::fmt::writer::MakeWriter): `stderr`/`File`
//! use the built-in writers, while the syslog backend sends one datagram per
//! event to the local syslog socket (`/dev/log`). Syslog relies on a Unix
//! datagram socket, so it is only available on Unix; selecting it elsewhere is
//! a clean error.
...
pub enum LoggerKind {
    ...
    File,
    /// Log to the system log via the local syslog socket (Unix only).
    Syslog,
}
...
/// cannot be opened, the syslog socket cannot be reached, or `syslog` is
/// selected on a non-Unix platform.
pub fn init(kind: LoggerKind, logfile: Option<&Path>, facility: &str) -> Result<()> {
    ...
    let writer = match kind {
        ...
        LoggerKind::Syslog => syslog_writer(facility)?,
    };
...
/// Builds the syslog [`MakeWriter`](tracing_subscriber::fmt::writer::MakeWriter)
/// on Unix.
///
/// # Errors
///
/// An error if the local syslog socket cannot be created or connected.
#[cfg(unix)]
fn syslog_writer(facility: &str) -> Result<BoxMakeWriter> {
    Ok(BoxMakeWriter::new(syslog::SyslogMaker::connect(facility)?))
}

/// The non-Unix stub: syslog needs a Unix datagram socket, so it is unavailable.
///
/// # Errors
///
/// Always returns an error explaining that syslog is Unix-only.
#[cfg(not(unix))]
fn syslog_writer(_facility: &str) -> Result<BoxMakeWriter> {
    anyhow::bail!("syslog logging is only available on Unix platforms; use --logger stderr or file")
}

/// The Unix syslog backend: one datagram per tracing event to `/dev/log`.
#[cfg(unix)]
mod syslog {
    ...
    impl SyslogMaker {
        ...
        /// # Errors
        ///
        /// An error if the socket cannot be created or connected (e.g. no
        ...
        fn connect_to(path: &Path, facility: &str) -> Result<Self> {
            let socket = std::os::unix::net::UnixDatagram::unbound()
                .context("creating the syslog socket")?;
            socket
                .connect(path)
    ...
    #[cfg(test)]
    mod tests {
        use super::{facility_code, severity, syslog_datagram, SyslogMaker};
        use std::io::Write;
