...
| `t/tasks/inventory/win32/**` | all `…/categories/*` (Windows path) | 🟡 | 🟡📦 | Windows inventory implemented via WMI/CIM (`collect_windows.go`) with ~25 categories; pure mappers unit-tested on Linux against CIM-JSON fixtures. **Batteries replays the two real `resources/win32/powercfg/*.xml` captures** (`testdata/powercfg/`), pinned to the `batteries.t` `_getBatteriesFromPowercfg` expectations; **USBDEVICES is pinned against the real vendored `usb.ids` DB using the `usb.t` vendor/device names**. Monitors reuse the shared (tested) EDID parser. Remaining: the WMI-method paths (process/user GetOwner), licenses, firewall-connection association. |
