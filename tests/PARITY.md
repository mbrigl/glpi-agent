...
| `t/tasks/inventory/win32/**` | all `…/categories/*` (Windows path) | 🟡 | 🟡📦 | Windows inventory implemented via WMI/CIM (`collect_windows.go`) with ~25 categories; pure mappers unit-tested on Linux against CIM-JSON fixtures. **Batteries replays the two real `resources/win32/powercfg/*.xml` captures** (`testdata/powercfg/`), pinned to the `batteries.t` `_getBatteriesFromPowercfg` expectations; **USBDEVICES is pinned against the real vendored `usb.ids` DB using the `usb.t` vendor/device names**; **the `decodeMicrosoftKey` license decoder is pinned against the real `license.t` binary DigitalProductID vectors**. Monitors reuse the shared (tested) EDID parser; process/user owners use the WMI GetOwner path. Remaining: process CPU/MEM perf counters, firewall-connection association, Adobe license source. |
| `t/tasks/inventory/macos/**` | all `…/categories/*` (macOS path) | 🟡 | ✅📦 | **All standard macOS categories implemented** (`collect_darwin.go`). The three core parsers (`system_profiler` text, plist-XML, ioreg) + every category mapper are pinned against the real `resources/macos/**` captures using the upstream `*.t` expectations: cpu.t, memory.t, videos.t, psu.t, networks.t, storages.t, softwares.t (exact 349/367 counts), usb.t, batteries.t, antivirus.t (Defender/Cortex/SentinelOne). Drives (df+diskutil) tested on synthetic input (no upstream fixtures). |
