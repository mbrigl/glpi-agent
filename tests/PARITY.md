...
| `t/tasks/inventory/linux/**` | `glpi-inventory-local` `categories::*` tests | ✅ | 📦 | Go ~28 Linux collectors with own parser tests; **LVM now replays the real `resources/lvm/**` pvs/vgs/lvs captures** (`testdata/lvm/`), which caught a real bug — `ParsePVS` dropped PVs with no volume group (empty trailing `vg_uuid`, 7 fields). Other `resources/linux/**` not yet replayed. |
