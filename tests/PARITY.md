...
| `t/tasks/inventory/linux/**` | `glpi-inventory-local` `categories::*` tests | ✅ | 📦 | Go ~28 Linux collectors with own parser tests; **LVM and rpm now replay the real `resources/linux/**` captures** (`testdata/{lvm,packaging}/`). Replaying LVM caught a real bug — `ParsePVS` dropped PVs with no volume group (empty trailing `vg_uuid`, 7 fields). dpkg is not replayed (Go parses `/var/lib/dpkg/status` stanzas, upstream uses `dpkg-query`). Other `resources/linux/**` not yet replayed. |
