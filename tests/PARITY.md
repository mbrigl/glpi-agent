...
| `t/tasks/inventory/macos/**` | all `…/categories/*` (macOS path) | 🟡 | 🟡📦 | macOS inventory started (`collect_darwin.go`): the `system_profiler` text parser + OS/hardware mappers are pinned against the real `resources/macos/system_profiler/{11.0-apple-M1,10.4-powerpc}` captures. More categories (cpu/memory/storages/networks/usb/…) follow. |
