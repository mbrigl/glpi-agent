...
| `t/tasks/inventory/win32/**` | all `…/categories/*` (Windows path) | 🟡 | 🟡📦 | Windows inventory implemented via WMI/CIM (`collect_windows.go`) with ~20 categories; pure mappers unit-tested on Linux against CIM-JSON fixtures. **Batteries replays the two real `resources/win32/powercfg/*.xml` captures** (`testdata/powercfg/`), pinned to the upstream `batteries.t` `_getBatteriesFromPowercfg` expectations. Remaining categories (monitors EDID-registry, firewall, usb) and the WMI-method paths (process/user GetOwner) are follow-on. |
