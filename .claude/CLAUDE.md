...
- **Phase 3 (NetInventory + MIBs):** core done — MIB framework, all 8 standard MIBs, NetInventory task and `sysObjectID`-gated vendor MIBs; the vendor-MIB set is being filled in high-value batches by porting the exact OIDs from the upstream Perl `MibSupport/*.pm` modules — 16 so far: Cisco, Juniper, Fortinet, Mikrotik, QNAP, Sophos, Hikvision, Eaton (ePDU/XUPS), Dell (PowerConnect/OS10), Citrix NetScaler, SonicWall, Ruckus, Zyxel, Raritan, Quantum, Infortrend.
