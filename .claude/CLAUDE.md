...
- **Phase 6 (Local inventory):** in progress — `glpi-inventory-local` collects a typed `Content` one category at a time (pure parsers over `/proc` / command output, unit-tested with fixtures; live collectors are thin platform wrappers under `cfg(target_os = "linux")`). Done (Linux): **os** (`/etc/os-release` + kernel + timezone), **cpu** (`/proc/cpuinfo`), **memory** (`dmidecode -t 17`), **software** (dpkg/rpm), **network** (`ip -o link`/`addr`), **hardware/bios** (`dmidecode` via the shared `categories::dmi` helper), **storage** (`lsblk -P`), **process** (`ps aux`). **`glpi-agent inventory`** runs end-to-end on Linux. Remaining: the other ~14 categories, Windows (COM/WMI), macOS, exotic platforms, and the `t/tasks/inventory/**` test migration.
