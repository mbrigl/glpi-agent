...
- **Phase 5 (CLI + daemon + HTTP):** core done — the **`glpi-agent` binary is runnable** with `inventory`, `netdiscovery`, `netinventory`, `inject` and `daemon` subcommands. Global `--conf-file` / `--conf-dir` load the layered configuration (defaults < agent.cfg < conf.d < env) via `glpi-core`'s `Loader`; the `inventory` command applies the resolved `no-category` filter (the other commands still take their settings from their own flags). `glpi-scheduler` (backoff + run-schedule) and `glpi-http` (embedded control server: `/status`, `/now`, `httpd-trust`) back the daemon. **Deferred within Phase 5:** the ToolBox UI pages, the proxy/SSL plugins (`glpi-plugins`), fork-based task isolation (the daemon runs tasks in-process on tokio instead), and server submission for the discovery/inventory results (only `inject` posts to a server so far).
