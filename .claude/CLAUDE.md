...
This repo (`glpi-agent`) is a **Rust rewrite of the Perl glpi-agent**. Status:

- **Phase 1 (Foundation):** complete — `glpi-core`, `glpi-transport`.
- **Phase 2 (NetDiscovery core):** complete — `glpi-discovery`.
- **Phase 3 (NetInventory + MIBs):** core done — MIB framework, all 8 standard MIBs, NetInventory task and `sysObjectID`-gated vendor MIBs; the vendor-MIB set is being filled in high-value batches by porting the exact OIDs from the upstream Perl `MibSupport/*.pm` modules — 20 so far: Cisco, Juniper, Fortinet, Mikrotik, QNAP, Sophos, Hikvision, Eaton (ePDU/XUPS), Dell (PowerConnect/OS10), Citrix NetScaler, SonicWall, Ruckus, Zyxel, Raritan, Quantum, Infortrend, and the printers OKI, Epson, Canon, Pantum (identity fields; vendor page-counter/supply enumeration not yet modelled).
- **Phase 5 (CLI + daemon + HTTP):** core done — the **`glpi-agent` binary is runnable** with `inventory`, `netdiscovery`, `netinventory`, `inject` and `daemon` subcommands. Global `--conf-file` / `--conf-dir` load the layered configuration (defaults < agent.cfg < conf.d < env) via `glpi-core`'s `Loader`; the `inventory` command applies the resolved `no-category` filter (the other commands still take their settings from their own flags). `glpi-scheduler` (backoff + run-schedule) and `glpi-http` (embedded control server: `/status`, `/now`, `httpd-trust`) back the daemon. `inventory --server <url>` (or the configured `server` list) submits the local inventory to GLPI via `GlpiClient::submit_inventory` (GLPI native `InventoryRequest` envelope; deviceid defaults to the hostname); `inject` posts inventory files. **Deferred within Phase 5:** the ToolBox UI pages, the proxy/SSL plugins (`glpi-plugins`), fork-based task isolation (the daemon runs tasks in-process on tokio instead), and server submission of NetDiscovery/NetInventory *results* (those still need a result→GLPI-protocol mapping).

- **Phase 6 (Local inventory):** Linux surface complete — `glpi-inventory-local` collects a typed `Content` one category at a time (pure parsers over `/proc`, `/sys` and command output, unit-tested with fixtures; live collectors are thin `cfg(target_os = "linux")` wrappers that degrade to empty when a tool/path is absent). **`glpi-agent inventory`** runs end-to-end on Linux. Categories done: **os** (os-release + kernel + timezone), **hardware/bios** (`dmidecode` 0/1/2/3 via the shared `categories::dmi` helper, incl. vmsystem via `systemd-detect-virt`), **cpu** (`/proc/cpuinfo`), **memory** (`dmidecode -t 17`), **storage** (`lsblk -P`), **network** (`ip -o link`/`addr`), **software** (dpkg/rpm), **process** (`ps aux`), **user** (`who`), **battery** (`/sys/class/power_supply`), **environment** (`std::env`), **controllers/pci** (`lspci -mm`), **video**/**sound** (`lspci`), **usb** (`lsusb`), **printer** (`lpstat -p`), **monitor** (EDID from `/sys/class/drm`), **antivirus** (Linux sensor markers); storage is enriched via `smartctl`. The emitted content is **schema-validated against a live GLPI server** (it accepts the inventory) and locked by a regression test (`tests/glpi_schema.rs`); a real `/proc/cpuinfo` capture seeds the fixture-replay pipeline. **Deferred** (need other hosts/assets, not buildable here): Windows (COM/WMI worker), macOS, exotic platforms (Solaris/HP-UX/AIX/FreeBSD), the `t/tasks/inventory/**` test migration + `resources/**` fixtures, and the remaining niche/credential-dependent categories (databases, hosted-VM/virtualization listing, `.ids`/EDID vendor-name lookup).
...
- **`glpi-discovery`** — the NetDiscovery core (Phase 2):
  ...
  - `snmp::mib` — the Phase 3 NetInventory framework: the `MibSupport` trait + `MibRegistry` (`with_standard` / `with_defaults`), the 8 standard MIBs (system, if, entity, printer, bridge, lldp, cdp, ip) building a `NetworkDevice` (info, ports, components, printer supplies), `sysObjectID`-gated vendor MIBs under `snmp::mib::vendor` (`sysobjectid_matches`; Cisco/Juniper/Fortinet so far), all tested via `WalkSession`,
  - `tasks::net_discovery` — `NetDiscoveryTask`: scans ranges, runs liveness + per-credential SNMP, emits classified `DiscoveredDevice` records (IEC 61850 merge hooks in here in Phase 4); `tasks::net_inventory` — `NetInventoryTask`: deep per-device SNMP inventory via the MIB registry.
