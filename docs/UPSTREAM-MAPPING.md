...
> NetInventory over SNMP via gosnmp), and **Phase 6** (Linux local inventory: 21
...
| --- | --- | --- | --- | --- |
| `Inventory.pm` | `glpi-inventory-local` | ✅ | `internal/{content,inventory}` | 🟡 document model + 21 Linux sections (bios, hardware, os, cpus, memories, networks, drives, storages, softwares, local_users/groups, envs, batteries, inputs, processes, usbdevices, controllers, videos, sounds, slots, ports); see the local-sections table. dmidecode/lspci/lvm-based categories and Windows/macOS pending |
...
> **Go:** Linux 🟡 (21 sections via `//go:build linux` collectors — see the local
