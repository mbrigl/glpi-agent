...
| `NetInventory.pm` | `glpi-discovery` | ✅ | `internal/discovery` | 🟡 generic properties + sysObjectID classification (embedded `sysobject.ids`) + SERIAL/FIRMWARE/MAC + IF-MIB PORTS + generic ENTITY-MIB COMPONENTS (incl. Dell/Cisco fix-ups) + NETWORKING port enrichment (TRUNK, AGGREGATE via LACP/PAgP, known-MAC FDB connections) via gosnmp + MibSupport (all 78 device/OS vendor modules + the SnmpFramework engine-id fallback classifier, incl. the run/getComponents device-mutation hooks; only ConfigurationPlugin — a config-time plugin loader — is unported) + **per-port VLANS** (Cisco VTP, 802.1Q egress/untagged bitmaps, LLDP fallbacks) + **CDP/LLDP/EDP neighbour discovery** (PORT.CONNECTIONS with the LLDP↔CDP↔EDP reconcile precedence) + the **per-VLAN FDB context switching** (re-read the dot1d FDB once per non-CDP VLAN via the `<community>@<vlan>` / `vlan-<n>` SNMP context). **NetInventory is feature-complete** vs the upstream port enrichments; only the config-time `ConfigurationPlugin` MibSupport loader is unported |
| `ESX.pm` | `glpi-vsphere` | ✅ | `internal/vsphere` | ✅ via govmomi |
| `RemoteInventory.pm` | `glpi-inventory-remote` | ✅ | `internal/remote` | ✅ SSH connect/exec + a remote Linux inventory that reuses the shared pure parsers over the SSH executor — **~22 sections**: OPERATINGSYSTEM (uname + os-release), HARDWARE (NAME/ARCH/MEMORY/SWAP), CPUS, **BIOS** (/sys/class/dmi/id), MEMORIES/SLOTS/PORTS (dmidecode), SOFTWARES (rpm → dpkg fallback), PHYSICAL/VOLUME/LOGICAL_VOLUMES (pvs/vgs/lvs), **LOCAL_USERS/LOCAL_GROUPS** (/etc/passwd+group), **USERS** (who), **INPUTS** (/proc/bus/input/devices), **PRINTERS** (cups.conf), **CONTROLLERS/VIDEOS/SOUNDS** (lspci), **DRIVES** (/proc/mounts + remote `df`), **NETWORKS** (remote `/sys/class/net` + `ip -o addr`), **FIREWALL** (ufw/firewalld), **ANTIVIRUS** (all the Linux detectors over SSH), **MONITORS** (base64-read DRM EDID), and BATTERIES/USBDEVICES/STORAGES via the `inventory.FS` abstraction (the Go counterpart of the `Tools::remote` file-read seam; an SSH-backed `remoteFS` reads the remote `/sys` over `cat`/`ls`). **The remote Linux inventory is now comprehensive (~26 sections).** Only PROCESSES (heavy per-process SSH) and VIRTUALMACHINES (tied to many build-tagged hypervisor helpers) are deferred, plus the **WinRM** transport + a Windows remote builder |
...
| macOS | `Inventory/MacOS/**` (25) | 🟡 implemented; some detail fields best-effort (`macos`) | ✅ **all standard MacOS inventory categories** mapped in the darwin collector (`collect_darwin.go`) via `system_profiler` (text + plist-XML), `sysctl`, `ioreg`, `ifconfig`/`networksetup`, `df`/`mount`/`diskutil`/`fdesetup` and the vendor AV CLIs; the three core parsers (`getSystemProfilerInfos` text, plist-XML, ioreg) + pure mappers are unit-tested on Linux against the real `resources/macos/**` captures. Done: operatingsystem (FULL_NAME/VERSION + uname kernel/arch), hardware (NAME + Hardware UUID, ioreg fallback), cpus (system_profiler + `sysctl machdep.cpu`: CORE/THREAD/NAME/MANUFACTURER/FAMILYNUMBER/MODEL/STEPPING/SPEED — pinned to cpu.t), memories (SPMemoryDataType DIMM slots + Apple-Silicon integrated-memory fallback + hardware MEMORY — pinned to memory.t), bios (SPHardwareDataType + ioreg fallbacks — SMANUFACTURER/SMODEL/SSN/BVERSION), videos (SPDisplaysDataType — CHIPSET/MEMORY/NAME/RESOLUTION/PCISLOT, pinned to videos.t), powersupplies (SPPowerDataType AC charger — pinned to psu.t), networks (`ifconfig -a` joined with `networksetup -listallhardwareports`: STATUS/DESCRIPTION/VIRTUALDEV/MACADDR/TYPE/IPADDRESS/IPADDRESS6/IPMASK/IPSUBNET/MTU/SPEED — pinned to networks.t; route-table DEFAULTGATEWAY/IPGATEWAY follow-on), storages (Apple **plist-XML** parser → SATA + NVMe disks: NAME/MANUFACTURER (`getCanonicalManufacturer` port)/INTERFACE/SERIAL/MODEL/FIRMWARE/DISKSIZE/TYPE/DESCRIPTION — pinned to storages.t), softwares (both the **text** and **XML** SPApplicationsDataType paths: NAME/VERSION/PUBLISHER (Obtained-from / Signed-by / Get-Info-String copyright / canonical-manufacturer heuristics) /INSTALLDATE (per-path `_formatDate` MM/DD vs `_getOffsetDate` DD/MM) /ARCH/SYSTEM_CATEGORY/USERNAME — **counts + spot-checks pinned to softwares.t** sample1/sample5), usb (**ioreg** `IOUSBDevice` parser → VENDORID/PRODUCTID (dec2hex)/SERIAL/NAME/CLASS/SUBCLASS, serial-dedup — pinned to usb.t IOUSBDevice1/2), sounds (SPAudioDataType), batteries (SPPowerDataType — pinned to batteries.t), hostname/uptime (Computer Name + arch/uptime), drives (`mount`/`df` + `diskutil info` join + FileVault), firewall (alf service + globalstate), the rest of the storage datatypes (DiscBurning/CardReader/USB/FireWire — DiscBurning+CardReader pinned to storages.t), antivirus (Defender/Cortex/SentinelOne/CrowdStrike/WithSecure detectors — Defender/Cortex/SentinelOne pinned to antivirus.t). Only un-ported MacOS module: `Inventory/MacOS/Hardware/*` battery-merge edge + route-table DEFAULTGATEWAY. |
