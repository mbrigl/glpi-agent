...
| `RemoteInventory.pm` | `glpi-inventory-remote` | ✅ | `internal/remote` | 🟡 SSH connect/exec + remote inventory reusing the shared pure parsers over the SSH executor: OPERATINGSYSTEM (uname + os-release), HARDWARE (NAME/ARCH/MEMORY/SWAP from /proc/meminfo), CPUS (/proc/cpuinfo), MEMORIES/SLOTS/PORTS (dmidecode), SOFTWARES (rpm → dpkg-status fallback), PHYSICAL/VOLUME/LOGICAL_VOLUMES (pvs/vgs/lvs), and the **sysfs-based BATTERIES/USBDEVICES/STORAGES** via the new `inventory.FS` abstraction (the Go counterpart of the `Tools::remote` file-read seam — `readSysLine`/glob route through it, an SSH-backed `remoteFS` reads the remote `/sys` over `cat`/`ls`). The remaining sysfs/proc collectors still live in the build-tagged `collect_linux.go` orchestration (networks/drives/users/processes/monitors) and need that un-tagging + the FS seam to run remotely; **WinRM** transport (`masterzen/winrm`) + a Windows remote builder are the other remaining pieces |
