...
| `RemoteInventory.pm` | `glpi-inventory-remote` | ✅ | `internal/remote` | ✅ SSH connect/exec + a remote Linux inventory that reuses the shared pure parsers over the SSH executor — **~22 sections**: OPERATINGSYSTEM (uname + os-release), HARDWARE (NAME/ARCH/MEMORY/SWAP), CPUS, **BIOS** (/sys/class/dmi/id), MEMORIES/SLOTS/PORTS (dmidecode), SOFTWARES (rpm → dpkg fallback), PHYSICAL/VOLUME/LOGICAL_VOLUMES (pvs/vgs/lvs), **LOCAL_USERS/LOCAL_GROUPS** (/etc/passwd+group), **USERS** (who), **INPUTS** (/proc/bus/input/devices), **PRINTERS** (cups.conf), **CONTROLLERS/VIDEOS/SOUNDS** (lspci), **DRIVES** (/proc/mounts + remote `df`), and BATTERIES/USBDEVICES/STORAGES via the `inventory.FS` abstraction (the Go counterpart of the `Tools::remote` file-read seam; an SSH-backed `remoteFS` reads the remote `/sys` over `cat`/`ls`). Remaining: NETWORKS (needs a remote `ip addr` path vs the local `net.Interfaces`), PROCESSES/MONITORS/VM/AV detectors (tied to local `/proc`-walking + build-tagged helpers), and the **WinRM** transport + a Windows remote builder |
