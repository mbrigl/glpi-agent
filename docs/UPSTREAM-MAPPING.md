...
| `NetInventory.pm` | `glpi-discovery` | ✅ | `internal/discovery` | 🟡 generic properties + sysObjectID classification (embedded `sysobject.ids`) + SERIAL/FIRMWARE/MAC + IF-MIB PORTS via gosnmp + MibSupport (16/80 vendor modules: Mikrotik, Ubnt, Dell, Fortinet, Cisco, Juniper, HP, Brother, Canon, Epson, Konica, Ricoh, Kyocera, Lexmark, Zebra); ongoing port |
...
| device classification | `mib/device.rs` | `discovery/classify` + `mibsupport` | ✅ Go (sysObjectID DB); MibSupport overrides 🟡 (framework + 16 vendors) |
...
> HP, Brother, Canon, Epson, Konica, Ricoh, Kyocera, Lexmark, Zebra **(16/80)**,
> each verbatim from the upstream `MibSupport/**` OIDs (not the Rust files).
> Matching covers sysObjectID, sysORID and privateoid rules. The remaining ~64
