...
| `RemoteInventory.pm` | `glpi-inventory-remote` | ✅ | `internal/remote` | 🟡 SSH connect/exec + remote inventory reusing the shared pure parsers over the SSH executor: OPERATINGSYSTEM (uname + os-release), HARDWARE (NAME/ARCH/MEMORY/SWAP from /proc/meminfo), CPUS (/proc/cpuinfo), MEMORIES/SLOTS/PORTS (dmidecode), SOFTWARES (rpm → dpkg-status fallback), PHYSICAL/VOLUME/LOGICAL_VOLUMES (pvs/vgs/lvs). The sysfs-only collectors (networks/drives/batteries/…) need the full filesystem abstraction (the upstream `Tools::remote` seam); **WinRM** transport (`masterzen/winrm`) + a Windows remote builder are the remaining pieces |
